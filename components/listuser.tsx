...
export default function ListUser({ users, filter, order }: { users: UserList[], filter: string, order: boolean }) {
    const [page, setPage]: [page: any, setPage: any] = useState<number>();
    ...
    useEffect(() => {
        ...
        if (typeof window !== 'undefined' && !window.localStorage.pagination) {
            ...
            PAGINATION = 0;
            setPage(PAGINATION);
            console.log("no exite pagination creamos")
            return;
        }
    ...
    const usersFilter: UserList[] = users.filter(user => {
        return user.name.toLowerCase().includes(filter.toLowerCase());
    });

    const auxUsers: UserList[] = [...usersFilter];
    const usersOrder: UserList[] = auxUsers.sort((a, b) => a.name.toLowerCase().localeCompare(b.name.toLowerCase()));
    const listUsers: Array<UserList[]> = [];
    let row: Array<UserList> = [];
    const usersForPagination: UserList[] = [];
    ...
    console.log(page);
    const maxpagination: number = listUsers.length - 1;
        ...
        return (
            <div className="flex-col justify-center items-center mt-20">
                <Image
                    ...
                    priority
                />
                <h2 className=" text-sm font-medium sm:text-xl sm:font-semibold text-center text-[#4B4B52]">No hay usuarios</h2>
            </div>
        )
    }
    ...
    return (
        <div className="flex-col w-full justify-center mt-5">
            <ul className="flex flex-col flex-start items-center w-full mt-5 mb-11 border-b border-gray-300 border-opa">
                {
                    page <= maxpagination ? listUsers[page].map((user: UserList, index: number) => {
                        return (
                            < li key={user.id} className={`px-3 flex w-full justify-around  py-4 md:px-8 gap-3 ${index % 2 ? '' : 'bg-[#FDF9F7]'} `}>
                                <Link href={`/profile/${user.id}`} className="w-11/12 flex justify-start gap-3 lg:hover:scale-105 lg:hover:translate-x-[30px] xl:hover:translate-x-[50px]">
                                    <Image
                                        src={user.photourl}
                                        alt="avatar user"
                    ...
                    }) : listUsers[maxpagination].map((user: UserList, index: number) => {
                        return (
                            < li key={user.id} className={`flex w-full  justify-around py-4 px-8 gap-3 ${index % 2 ? '' : 'bg-[#FDF9F7]'} `}>
                                <Link href={`/profile/${user.id}`} className="w-11/12 flex justify-start gap-3 lg:hover:scale-105 lg:hover:translate-x-[30px] xl:hover:translate-x-[50px]">
                                    <Image
                                        src={user.photourl}
                                        alt="avatar user"
            ...
            <div className="flex justify-center items-center gap-2">
                <ul className="inline-flex justify-center gap-2 items-center ">
                    ...
                    <li className={`flex justify-center items-center py-1 bg-[#FDF9F7] border rounded w-8 ${page > 0 ? 'border-[#FFC592] hover:scale-110' : 'border-[#D7D7D7]'}`}> <button className="hover:scale-110" disabled={page > 0 ? false : true} onClick={() => { setPage((prevPage: number) => prevPage - 1) }}>
                        <Image
                            src={page > 0 ? "/ordenador izq orange.svg" : "/ordenador izq grey.svg"}
                            alt="Avatar Usuario"
                    ...
                    <li className={`flex justify-center items-center py-1 bg-[#FDF9F7] border rounded w-8 ${maxpagination > page ? 'border-[#FFC592] hover:scale-110' : 'border-[#D7D7D7]'}`}> <button className="inline-flex" disabled={page >= maxpagination ? true : false} onClick={() => { setPage(maxpagination) }}>
                        <Image
                            src={maxpagination > page ? "/ordenador der orange.svg" : "/ordenador der grey.svg"}
                            alt="Avatar Usuario"
                            className="flex-shrink-0 text-[#D7D7D7] "
